...
class PrismaticVLM(VLM):
    ...
    def forward(
        ...
        output_vis_tokens: Optional[bool] = False,
        return_dict: Optional[bool] = None,
        multimodal_indices: Optional[torch.LongTensor] = None,
    ) -> CausalLMOutputWithPast:
        """Run a forward pass through the VLM, returning a CausalLMOutputWithPast instance (contains loss)."""

        # print(pixel_values.keys()) # (['dino', 'siglip'])
        ...
        multimodal_embeddings = torch.cat(
            [
                ...
                projected_patch_embeddings,                         # visual features
                input_embeddings[multimodal_indices, 1:, :],        # instruction (language)
            ],
            dim=1,
        )
        ...
        # print('fused_embdddings ', fused_embeddings.shape)
        ...
        return {'vis_tokens':patch_features, 'llm_output':llm_output} if output_vis_tokens else llm_output
