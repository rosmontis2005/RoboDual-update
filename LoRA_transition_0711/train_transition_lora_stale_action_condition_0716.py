#!/usr/bin/env python3
"""Train a stale action-condition LoRA with front-loaded chunk supervision.

Only stale samples that match the online age>=8 gate receive action-label
supervision. Normal samples preserve the frozen base. LoRA is restricted to the
slow-condition path instead of temporal action projections, and training is
balanced across categories and source trajectories.
"""
...
DEFAULT_OUTPUT_DIR = EXPERIMENT_ROOT / "lora_runs" / "transition_lora_v13_stale_action_condition"
...
def front_weighted_mse(
    prediction: torch.Tensor,
    target: torch.Tensor,
    front_action_steps: int,
    front_action_weight: float,
) -> torch.Tensor:
    if prediction.shape != target.shape or prediction.ndim != 3:
        raise ValueError("prediction and target must have identical [B,T,D] shapes")
    if not 0 < front_action_steps <= prediction.shape[1] or front_action_weight <= 1:
        raise ValueError("front action weighting parameters are invalid")
    step_weights = torch.ones(
        prediction.shape[1], device=prediction.device, dtype=prediction.dtype
    )
    step_weights[:front_action_steps] = front_action_weight
    squared_error = (prediction - target).square()
    return (
        squared_error * step_weights.view(1, -1, 1)
    ).sum() / (prediction.shape[0] * prediction.shape[2] * step_weights.sum())
...
def compute_preserved_objective(policy, batch: dict, args, *, need_teacher: bool = True) -> dict:
    ...
    supervised = front_weighted_mse(
        prediction, target, args.front_action_steps, args.front_action_weight
    )
    ...
    objective = supervised_weight * supervised + preservation_weight * drift
    return {
        "loss": objective,
...
def trajectory_category_balanced_sampler(
    dataset: TransitionManifestDataset, seed: int
) -> WeightedRandomSampler:
    """Give each category equal mass, then each trajectory equal mass within it."""

    pair_counts = Counter(
        (sample["category"], sample["trajectory_id"]) for sample in dataset.samples
    )
    trajectories_by_category = defaultdict(set)
    for category, trajectory_id in pair_counts:
        trajectories_by_category[category].add(trajectory_id)
    if set(trajectories_by_category) != set(CATEGORIES):
        raise ValueError(
            "balanced sampler category mismatch: "
            f"expected={sorted(CATEGORIES)}, actual={sorted(trajectories_by_category)}"
        )
    weights = []
    for sample in dataset.samples:
        key = (sample["category"], sample["trajectory_id"])
        category_trajectories = len(trajectories_by_category[sample["category"]])
        weights.append(1.0 / (len(CATEGORIES) * category_trajectories * pair_counts[key]))
...
def checkpoint_selection_score(metrics: dict, args) -> float:
    return float(
        args.selection_normal_drift_weight * metrics["normal_drift"]
        + args.selection_overall_drift_weight * metrics["overall_drift"]
        + args.selection_normal_gripper_drift_weight * metrics["normal_drift_gripper"]
    )
...
def train(args: argparse.Namespace) -> dict:
    ...
    train_sampler = trajectory_category_balanced_sampler(train_dataset, args.seed)
    ...
    forbidden = (
        "visual_adapter", "depth_adapter", "gripper_", "proprio_embedder",
        ".mlp.", ".attn_temporal.", "final_layer",
    )
    ...
    config = {
        ...
        "train_sampling": "equal category mass, then equal trajectory mass within category",
        "deployment_gate": "slow_age_after >= 8 and reference exhausted",
        "supervision": (
            f"diffusion MSE with first {args.front_action_steps} action tokens weighted "
            f"{args.front_action_weight}x"
        ),
    ...
    baseline_transition_loss = transition_supervised_loss(baseline_validation)
    best_validation = baseline_transition_loss
    ...
    while optimizer_step < args.max_steps:
        for batch in train_loader:
            ...
            if should_validate:
                ...
                transition_improvement = baseline_transition_loss - transition_loss
                selection_score = checkpoint_selection_score(validation, args)
                ...
                selection_eligible = (
                    constraints_met
                    and transition_improvement >= args.selection_min_transition_improvement
                )
                loss_improved = transition_loss < best_validation - args.early_stopping_min_delta
                loss_tied = abs(transition_loss - best_validation) <= args.early_stopping_min_delta
                drift_improved = (
                    selection_score < best_selection_score - args.selection_score_min_delta
                )
                improved = selection_eligible and (
                    best_step == 0 or loss_improved or (loss_tied and drift_improved)
                )
                if improved:
    ...
    summary = {
        **config,
        ...
        "best_validation_metric": (
            "minimum stale supervised loss subject to preservation constraints and minimum improvement; "
            "weighted prediction drift breaks loss ties"
        ),
        "baseline_transition_supervised_loss": baseline_transition_loss,
        "best_transition_improvement": baseline_transition_loss - best_validation,
        "best_checkpoint_selection_score": (
            best_selection_score if best_step > 0 else None
        ),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lora_rank", type=int, default=4, choices=[4])
    ...
    parser.add_argument("--min_steps_before_early_stopping", type=int, default=1200)
    ...
    parser.add_argument("--transition_preservation_weight", type=float, default=0.1)
    ...
    parser.add_argument("--selection_min_transition_improvement", type=float, default=2.0e-4)
    parser.add_argument("--selection_score_min_delta", type=float, default=0.0)
    parser.add_argument("--selection_normal_drift_weight", type=float, default=1.0)
    parser.add_argument("--selection_overall_drift_weight", type=float, default=1.0)
    parser.add_argument("--selection_normal_gripper_drift_weight", type=float, default=2.0)
    ...
    if args.normal_supervised_weight != 0:
        parser.error("V13 fixes --normal_supervised_weight=0 for deployment-matched training")
    if args.empty_ref_after_age != 8:
        parser.error("V13 fixes --empty_ref_after_age=8 to match the online stale gate")
    if args.selection_min_transition_improvement <= 0 or args.selection_score_min_delta < 0:
        parser.error("selection improvement must be positive and score delta non-negative")
    if args.front_action_weight <= 1:
        parser.error("--front_action_weight must be greater than 1")
    selection_weights = (
        args.selection_normal_drift_weight,
        args.selection_overall_drift_weight,
        args.selection_normal_gripper_drift_weight,
    )
    if any(weight < 0 for weight in selection_weights) or not any(selection_weights):
        parser.error("selection drift weights must be non-negative with at least one positive")
    return args
