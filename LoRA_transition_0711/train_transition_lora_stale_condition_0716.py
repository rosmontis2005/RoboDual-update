#!/usr/bin/env python3
"""Train a deployment-matched stale-condition LoRA for the fast specialist.
...
DEFAULT_OUTPUT_DIR = EXPERIMENT_ROOT / "lora_runs" / "transition_lora_v12_stale_condition"
...
def train(args: argparse.Namespace) -> dict:
    ...
    forbidden = (
        ...
        ".mlp.", ".attn_temporal.", "final_layer", "model.x_embedder",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--transition_preservation_weight", type=float, default=0.25)
    ...
    parser.add_argument("--selection_min_transition_improvement", type=float, default=2.5e-4)
    ...
    if args.normal_supervised_weight != 0:
        parser.error("V12 fixes --normal_supervised_weight=0 for deployment-matched training")
    if args.empty_ref_after_age != 8:
        parser.error("V12 fixes --empty_ref_after_age=8 to match the online stale gate")
