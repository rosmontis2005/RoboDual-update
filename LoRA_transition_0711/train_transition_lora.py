#!/usr/bin/env python3
"""Train the Legato-like fast specialist on persisted transition conditions."""
...
DEFAULT_OUTPUT_DIR = EXPERIMENT_ROOT / "lora_runs" / "transition_history_lora_v2_repaired"
...
DEFAULT_LORA_TARGETS = (
    ...
    *(f"model.blocks.{index}.attn_temporal.qkv" for index in range(6)),
    *(f"model.blocks.{index}.attn_temporal.proj" for index in range(6)),
)
...
def compute_policy_loss(policy, batch: dict, bf16: bool) -> torch.Tensor:
...
def evaluate_loss(policy, dataloader, device: torch.device, bf16: bool, seed: int) -> dict:
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            loss = float(compute_policy_loss(policy, batch, bf16).detach().cpu())
            batch_size = len(batch["category"])
            sums["overall"] += loss * batch_size
            totals["overall"] += batch_size
            # compute_loss returns a batch mean; use it as a stable category
            # estimate for every category represented in this small batch.
    ...
    return {key: sums[key] / totals[key] for key in totals}
...
def train(args: argparse.Namespace) -> dict:
    ...
    history_adapter.requires_grad_(True)
    ...
    baseline_validation = evaluate_loss(
        policy, validation_loader, device, args.bf16, args.validation_seed
    )
    ...
    while optimizer_step < args.max_steps:
        for batch in train_loader:
            batch = move_batch(batch, device)
            loss = compute_policy_loss(policy, batch, args.bf16)
            ...
            if should_validate:
                ...
                normal_ratio = validation["normal"] / baseline_validation["normal"]
                val_metric = {
                    "event": "validation", "step": optimizer_step,
                    **{f"loss_{key}": value for key, value in validation.items()},
                    "normal_vs_base_ratio": normal_ratio,
                }
                with metrics_path.open("a") as file:
                    file.write(json.dumps(val_metric, sort_keys=True) + "\n")
                improved_unconstrained = (
                    validation["overall"] < best_unconstrained_validation - args.early_stopping_min_delta
                )
                if improved_unconstrained:
                    best_unconstrained_validation = validation["overall"]
                ...
                improved = (
                    normal_ratio <= args.max_normal_loss_ratio
                    and validation["overall"] < best_validation - args.early_stopping_min_delta
                )
                if improved:
                    best_validation = validation["overall"]
                ...
                elif best_step > 0 or not improved_unconstrained:
                    stale_evaluations += 1
                else:
                    stale_evaluations = 0
                if normal_ratio > args.max_normal_loss_ratio:
                    print(
                        f"[warning] normal validation loss ratio {normal_ratio:.4f} exceeds "
                        f"limit {args.max_normal_loss_ratio:.4f}", flush=True,
                    )
            ...
            if optimizer_step >= args.max_steps:
                break
        if optimizer_step >= args.max_steps or stale_evaluations >= args.early_stopping_patience:
            break
    ...
    best_test = evaluate_loss(policy, test_loader, device, args.bf16, args.validation_seed + 1)
    ...
    summary = {
        **config,
        ...
        "stopped_early": stale_evaluations >= args.early_stopping_patience,
        "best_step": best_step,
        ...
        "selected_best_meets_normal_constraint": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max_normal_loss_ratio", type=float, default=1.05)
