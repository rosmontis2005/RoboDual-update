#!/usr/bin/env python3
"""Train and drift-select a narrow transition LoRA for the fast specialist.
...
Checkpoint selection first requires a real transition-loss gain and then picks
the eligible candidate with the smallest weighted prediction drift.
"""
...
DEFAULT_OUTPUT_DIR = EXPERIMENT_ROOT / "lora_runs" / "transition_lora_v5_drift_selected"
...
def train(args: argparse.Namespace) -> dict:
    ...
    while optimizer_step < args.max_steps:
        for batch in train_loader:
            ...
            if should_validate:
                ...
                improved = (
                    selection_eligible
                    and selection_score < best_selection_score - args.selection_score_min_delta
                )
                if improved:
    ...
    summary = {
        **config,
        ...
        "best_validation_metric": "minimum weighted prediction drift after minimum transition improvement",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min_steps_before_early_stopping", type=int, default=1500)
    ...
    parser.add_argument("--selection_min_transition_improvement", type=float, default=3e-6)
